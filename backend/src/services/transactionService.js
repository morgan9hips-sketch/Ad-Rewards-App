"use strict";
var __awaiter = (this && this.__awaiter) || function (thisArg, _arguments, P, generator) {
    function adopt(value) { return value instanceof P ? value : new P(function (resolve) { resolve(value); }); }
    return new (P || (P = Promise))(function (resolve, reject) {
        function fulfilled(value) { try { step(generator.next(value)); } catch (e) { reject(e); } }
        function rejected(value) { try { step(generator["throw"](value)); } catch (e) { reject(e); } }
        function step(result) { result.done ? resolve(result.value) : adopt(result.value).then(fulfilled, rejected); }
        step((generator = generator.apply(thisArg, _arguments || [])).next());
    });
};
var __generator = (this && this.__generator) || function (thisArg, body) {
    var _ = { label: 0, sent: function() { if (t[0] & 1) throw t[1]; return t[1]; }, trys: [], ops: [] }, f, y, t, g = Object.create((typeof Iterator === "function" ? Iterator : Object).prototype);
    return g.next = verb(0), g["throw"] = verb(1), g["return"] = verb(2), typeof Symbol === "function" && (g[Symbol.iterator] = function() { return this; }), g;
    function verb(n) { return function (v) { return step([n, v]); }; }
    function step(op) {
        if (f) throw new TypeError("Generator is already executing.");
        while (g && (g = 0, op[0] && (_ = 0)), _) try {
            if (f = 1, y && (t = op[0] & 2 ? y["return"] : op[0] ? y["throw"] || ((t = y["return"]) && t.call(y), 0) : y.next) && !(t = t.call(y, op[1])).done) return t;
            if (y = 0, t) op = [op[0] & 2, t.value];
            switch (op[0]) {
                case 0: case 1: t = op; break;
                case 4: _.label++; return { value: op[1], done: false };
                case 5: _.label++; y = op[1]; op = [0]; continue;
                case 7: op = _.ops.pop(); _.trys.pop(); continue;
                default:
                    if (!(t = _.trys, t = t.length > 0 && t[t.length - 1]) && (op[0] === 6 || op[0] === 2)) { _ = 0; continue; }
                    if (op[0] === 3 && (!t || (op[1] > t[0] && op[1] < t[3]))) { _.label = op[1]; break; }
                    if (op[0] === 6 && _.label < t[1]) { _.label = t[1]; t = op; break; }
                    if (t && _.label < t[2]) { _.label = t[2]; _.ops.push(op); break; }
                    if (t[2]) _.ops.pop();
                    _.trys.pop(); continue;
            }
            op = body.call(thisArg, _);
        } catch (e) { op = [6, e]; y = 0; } finally { f = t = 0; }
        if (op[0] & 5) throw op[1]; return { value: op[0] ? op[1] : void 0, done: true };
    }
};
Object.defineProperty(exports, "__esModule", { value: true });
exports.createTransaction = createTransaction;
exports.awardCoins = awardCoins;
exports.convertCoinsToUSD = convertCoinsToUSD;
exports.processWithdrawal = processWithdrawal;
exports.getUserTransactions = getUserTransactions;
var client_1 = require("@prisma/client");
var prisma = new client_1.PrismaClient();
/**
 * Create a transaction record with current balance snapshot
 * This should be called within a database transaction
 */
function createTransaction(params, tx) {
    return __awaiter(this, void 0, void 0, function () {
        var client, user, error_1;
        return __generator(this, function (_a) {
            switch (_a.label) {
                case 0:
                    client = tx || prisma;
                    _a.label = 1;
                case 1:
                    _a.trys.push([1, 4, , 5]);
                    return [4 /*yield*/, client.userProfile.findUnique({
                            where: { userId: params.userId },
                            select: {
                                coinsBalance: true,
                                cashBalanceUsd: true
                            },
                        })];
                case 2:
                    user = _a.sent();
                    if (!user) {
                        throw new Error("User ".concat(params.userId, " not found"));
                    }
                    // Create transaction record
                    return [4 /*yield*/, client.transaction.create({
                            data: {
                                userId: params.userId,
                                type: params.type,
                                coinsChange: params.coinsChange || BigInt(0),
                                cashChangeUsd: params.cashChangeUsd || 0,
                                coinsBalanceAfter: user.coinsBalance,
                                cashBalanceAfterUsd: user.cashBalanceUsd,
                                description: params.description,
                                referenceId: params.referenceId,
                                referenceType: params.referenceType,
                            },
                        })];
                case 3:
                    // Create transaction record
                    _a.sent();
                    return [3 /*break*/, 5];
                case 4:
                    error_1 = _a.sent();
                    console.error('Error creating transaction:', error_1);
                    throw error_1;
                case 5: return [2 /*return*/];
            }
        });
    });
}
/**
 * Award coins to a user and create transaction record
 * Returns the updated user profile
 */
function awardCoins(userId, coins, description, referenceId, referenceType) {
    return __awaiter(this, void 0, void 0, function () {
        var _this = this;
        return __generator(this, function (_a) {
            switch (_a.label) {
                case 0: return [4 /*yield*/, prisma.$transaction(function (tx) { return __awaiter(_this, void 0, void 0, function () {
                        var user;
                        return __generator(this, function (_a) {
                            switch (_a.label) {
                                case 0: return [4 /*yield*/, tx.userProfile.update({
                                        where: { userId: userId },
                                        data: {
                                            coinsBalance: { increment: BigInt(coins) },
                                            totalCoinsEarned: { increment: BigInt(coins) },
                                        },
                                    })
                                    // Create transaction record
                                ];
                                case 1:
                                    user = _a.sent();
                                    // Create transaction record
                                    return [4 /*yield*/, createTransaction({
                                            userId: userId,
                                            type: 'coin_earned',
                                            coinsChange: BigInt(coins),
                                            description: description,
                                            referenceId: referenceId,
                                            referenceType: referenceType,
                                        }, tx)];
                                case 2:
                                    // Create transaction record
                                    _a.sent();
                                    return [2 /*return*/, user];
                            }
                        });
                    }); })];
                case 1: return [2 /*return*/, _a.sent()];
            }
        });
    });
}
/**
 * Convert coins to cash and create transaction record
 * Used during monthly conversion process
 */
function convertCoinsToUSD(userId, coins, cashUsd, conversionId, tx) {
    return __awaiter(this, void 0, void 0, function () {
        return __generator(this, function (_a) {
            switch (_a.label) {
                case 0: 
                // Update user balances
                return [4 /*yield*/, tx.userProfile.update({
                        where: { userId: userId },
                        data: {
                            coinsBalance: { decrement: coins },
                            cashBalanceUsd: { increment: cashUsd },
                            totalCashEarnedUsd: { increment: cashUsd },
                        },
                    })
                    // Create transaction record
                ];
                case 1:
                    // Update user balances
                    _a.sent();
                    // Create transaction record
                    return [4 /*yield*/, createTransaction({
                            userId: userId,
                            type: 'coin_conversion',
                            coinsChange: -coins,
                            cashChangeUsd: cashUsd,
                            description: "Monthly coin conversion",
                            referenceId: conversionId,
                            referenceType: 'coin_conversion',
                        }, tx)];
                case 2:
                    // Create transaction record
                    _a.sent();
                    return [2 /*return*/];
            }
        });
    });
}
/**
 * Process withdrawal and create transaction record
 */
function processWithdrawal(userId, amountUsd, withdrawalId, tx) {
    return __awaiter(this, void 0, void 0, function () {
        return __generator(this, function (_a) {
            switch (_a.label) {
                case 0: 
                // Update user balances
                return [4 /*yield*/, tx.userProfile.update({
                        where: { userId: userId },
                        data: {
                            cashBalanceUsd: { decrement: amountUsd },
                            totalWithdrawnUsd: { increment: amountUsd },
                        },
                    })
                    // Create transaction record
                ];
                case 1:
                    // Update user balances
                    _a.sent();
                    // Create transaction record
                    return [4 /*yield*/, createTransaction({
                            userId: userId,
                            type: 'withdrawal',
                            cashChangeUsd: -amountUsd,
                            description: "Withdrawal via PayPal",
                            referenceType: 'withdrawal',
                        }, tx)];
                case 2:
                    // Create transaction record
                    _a.sent();
                    return [2 /*return*/];
            }
        });
    });
}
/**
 * Get user transaction history with pagination
 */
function getUserTransactions(userId_1) {
    return __awaiter(this, arguments, void 0, function (userId, page, perPage, type) {
        var skip, where, _a, transactions, total;
        if (page === void 0) { page = 1; }
        if (perPage === void 0) { perPage = 20; }
        return __generator(this, function (_b) {
            switch (_b.label) {
                case 0:
                    skip = (page - 1) * perPage;
                    where = { userId: userId };
                    if (type) {
                        where.type = type;
                    }
                    return [4 /*yield*/, Promise.all([
                            prisma.transaction.findMany({
                                where: where,
                                orderBy: { createdAt: 'desc' },
                                take: perPage,
                                skip: skip,
                            }),
                            prisma.transaction.count({ where: where }),
                        ])];
                case 1:
                    _a = _b.sent(), transactions = _a[0], total = _a[1];
                    return [2 /*return*/, {
                            transactions: transactions,
                            total: total,
                            pages: Math.ceil(total / perPage),
                        }];
            }
        });
    });
}

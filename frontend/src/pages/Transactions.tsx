  ...
  return (
    <div className="container mx-auto px-4 py-6 pb-24">
      ...
      <div className="flex gap-2 mb-6 overflow-x-auto pb-2">
        {['all', 'coin_earned', 'coin_conversion', 'withdrawal'].map((filterType) => (
          <button
            key={filterType}
            onClick={() => {
          ...
          >
            {filterType === 'all' ? 'All' : getTypeLabel(filterType).replace(/[🪙💱💸⚙️🏆]/g, '').trim()}
          </button>
        ))}
      </div>
              ...
              {transactions.map((tx) => (
                ...
                >
                  ...
                  <div className="text-right ml-4">
                    ...
                    {tx.cashBalanceAfterUsd && parseFloat(tx.cashBalanceAfterUsd) > 0 && (
                      <p className="text-xs text-gray-500">
                        ${parseFloat(tx.cashBalanceAfterUsd).toFixed(2)}
                      </p>
                    )}
                  </div>
                </div>
